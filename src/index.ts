...
app.post('/expenses', async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      ...
      [user_id, category_id || null, amount, description || null, date || null]
    );
  ...
  try {
    const result = await pool.query(
      `UPDATE expenses SET amount = COALESCE($1, amount), description = COALESCE($2, description),
       category_id = COALESCE($3, category_id), date = COALESCE($4, date)
       WHERE id = $5 RETURNING *`,
      [amount, description, category_id, date, req.params.id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Expense not found' });
    }
